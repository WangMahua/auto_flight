...
MISSION::MISSION(){
    ...
    /* ex : mission_insert("2", "0", 1.0, 1.0, 0.0); */
...
void MISSION::print_current_state(){
    cout << " cur_task : " << cur_task <<endl;
    string mode = mission_list[cur_task].mode;
    if(mode == "1"){
        cout << "takeoff " << endl;
    }else if(mode == "2"){
        cout << "waypoint ->"<<" target_x : " << mission_list[cur_task].data1 << " target_y : " << mission_list[cur_task].data2 << endl;
    }else if(mode == "3"){
        cout << "landing " << endl;
    }else{
        cout << "wait cmd "<<endl;
    ...
    cout << " total task num : " << final_task <<endl;
    cout << " === " << endl;
}
